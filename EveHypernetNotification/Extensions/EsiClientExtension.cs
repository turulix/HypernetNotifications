...
public static class EsiClientExtension
{
    ...
    public static async Task<float> GetMaxBuyOrderPriceAsync(
    ...
    )
    {
        ...
        return (float)orders.Data.Where(order => order.LocationId == locationId).Max(order => order.Price);
    }

    public static async Task<float> GetMinSellOrderPriceAsync(
        this EsiClient client,
        int typeId,
        long locationId = 60003760,
        int regionId = 10000002
    )
    {
        ...
        return (float)orders.Data.Where(order => order.LocationId == locationId).Min(order => order.Price);
    }
